...
export default function ProfilePage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!uploadData) {
        throw new Error('アップロードは成功しましたが、ファイル情報が返されませんでした')
      }
  ...
  const updateProfile = async (field: string, value: string) => {
    ...
    try {
      console.log(`Updating profile field '${field}' for user ${user.id}`)
      ...
      alert(`プロフィールの更新に失敗しました: ${error instanceof Error ? error.message : 'Unknown error'}`)
    }
  }
